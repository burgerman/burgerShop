            ...
            Init: function (isshow) {
                ...
                t.btns.forEach(function (btn) {
                    ...
                    if (btn.id != "closebtn") {
                    	...
                    	///////////////扩展内容可以自定closebtn按钮显示的内容不写text则使用默认内容，否则使用text内容显示////////////////////
                    	var closebtn_text = bootstrapEx.language.Modal.closebtn();
                    	if(btn.text != null && btn.text != ""){
                    		closebtn_text = btn.text;
                    	}
                    	_b = $('<button class="btn btn-default" type="button" data-dismiss="modal" aria-hidden="true" id="' + btn.id + '">' + closebtn_text + '</button>');
                    	///////////////扩展内容可以自定closebtn按钮显示的内容不写text则使用默认内容，否则使用text内容显示////////////////////
